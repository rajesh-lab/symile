...
class BaseDataModule(pl.LightningDataModule):
    ...
    def update_audio_path(self, df, data_dir):
        """
        Update audio_path in dataframe to use `data_dir` as parent path. Note
        this is only for templates 2, 3, and 4, since they use audio that was
        generated by the `generate_data.py` script.
        """
        df["audio_path"] = df.apply(
            lambda r: data_dir / r.audio_path if r.template in [2, 3, 4] else r.audio_path,
            axis=1
        )
        return df
...
class PretrainDataModule(BaseDataModule):
    ...
    def setup(self, stage):
        ...
        df_train = self.update_audio_path(df_train, self.args.data_dir)
        ...
        df_val = self.update_audio_path(df_val, self.args.data_dir)
...
class SupportClfDataModule(BaseDataModule):
    ...
    def setup(self, stage):
        ...
        df_test = self.update_audio_path(df_test, self.args.data_dir)
