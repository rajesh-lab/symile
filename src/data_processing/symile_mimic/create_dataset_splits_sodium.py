...
def sample_negative_candidates(df, candidate_n, seed=None):
    ...
    for i in range(len(df)):
        ...
        neg_candidates = remaining_df.sample(n=candidate_n-1, random_state=seed)
...
if __name__ == '__main__':
    ...
    COLS_TO_KEEP = ['subject_id', 'hadm_id', 'cxr_path', 'Atelectasis', 'Cardiomegaly',
       'Edema', 'Lung Opacity', 'No Finding', 'Pleural Effusion', 'ecg_path', '50983']

    df = df[COLS_TO_KEEP]
    df = df.rename(columns={'50983': 'sodium'})
    # .dropna(subset=['sodium'])
    ...
    ecdf = NaNAwareECDF(train_df["sodium"])
    train_df["sodium" + "_percentile"] = ecdf(train_df["sodium"])
    val_df["sodium" + "_percentile"] = ecdf(val_df["sodium"])
    test_df["sodium" + "_percentile"] = ecdf(test_df["sodium"])

    save_mean_percentiles(train_df, args)

    # sample negative candidates for each test sample
    val_retrieval_df = sample_negative_candidates(val_df, args.candidate_n, seed=seed)
